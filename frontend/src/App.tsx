...
function AnalyticsPage({ analytics }: { analytics: Analytics | null }) {
  ...
  const partner = analytics.piarflow;
  const partnerMax = Math.max(...partner.days.map((item) => Math.max(item.sold_subs, item.earned)), 1);
  const money = new Intl.NumberFormat("ru-RU", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const partnerFunnel = [["Показан барьер", partner.barrier_users], ["Получили задания", partner.task_users], ["Проверили", partner.check_users], ["Допущены", partner.completed_users]] as const;
  return <section className="page-stack"><div className="page-intro"><div><p className="eyebrow">PRODUCT ANALYTICS</p><h2>Бот и подключение</h2><p>События без хранения ссылок подписки: старты бота, переходы на сайт, импорт в HAPP и выдачи VPN.</p></div><span className="status-dot">14 ДНЕЙ</span></div>
    <div className="metric-grid">{cards.map(([label, value, description, Icon, tone]) => <article className="metric-card" key={label}><div className={`metric-icon ${tone}`}><Icon size={20}/></div><p>{label}</p><strong>{value}</strong><span>{description}</span></article>)}</div>
    <article className="panel piarflow-panel"><div className="panel-head"><div><p className="eyebrow">PIARFLOW ACCESS</p><h3>Спонсоры и обязательный вход</h3></div><span className={`status-dot ${partner.provider_active ? "" : "warning"}`}>{partner.provider_active ? `@${partner.username || "bot"} · ACTIVE` : partner.enabled ? "ОЖИДАНИЕ API" : "ВЫКЛЮЧЕН"}</span></div>
      <div className="piarflow-summary"><div><small>Заработано</small><strong>{money.format(partner.earned)} ₽</strong><span>{partner.sold_subs} оплаченных подписок</span></div><div><small>Не засчитано</small><strong>{partner.not_counted}</strong><span>выполнений без начисления</span></div><div><small>Текущий барьер</small><strong>{partner.max_sponsors || 3}</strong><span>сброс PiarFlow: {partner.reset_time || 0} мин.</span></div><div><small>Синхронизация</small><strong>{partner.last_synced_at ? formatDate(partner.last_synced_at) : "—"}</strong><span>{partner.last_error || "ошибок нет"}</span></div></div>
      <div className="piarflow-funnel">{partnerFunnel.map(([label, value], index) => <div key={label}><small>{label}</small><strong>{value}</strong><span>{index === 2 ? `${partner.check_conversion}% от показа` : index === 3 ? `${partner.completion_conversion}% от показа` : "уникальные пользователи"}</span></div>)}</div>
      <div className="piarflow-state"><span>Ожидают <b>{partner.pending_users}</b></span><span>Без заданий <b>{partner.deferred_users}</b></span><span>Отписались <b>{partner.unsubscribed_users}</b></span><span>Проверок <b>{partner.check_attempts}</b></span><span>Среднее до допуска <b>{partner.average_checks_to_complete}</b></span><span>API ошибок <b>{partner.api_errors}</b></span><span>Нет инвентаря <b>{partner.no_inventory}</b></span><span>Отозвано устройств <b>{partner.revoked_devices}</b></span></div>
      <div className="piarflow-bars">{partner.days.length ? partner.days.map((day) => <div className="piarflow-day" key={day.date} title={`${day.date}: продажи ${day.sold_subs}, доход ${money.format(day.earned)} ₽`}><div><i className="sold" style={{height:`${day.sold_subs / partnerMax * 100}%`}}/><i className="earned" style={{height:`${day.earned / partnerMax * 100}%`}}/></div><small>{new Intl.DateTimeFormat("ru-RU", {day:"numeric", month:"short"}).format(new Date(`${day.date}T00:00:00Z`))}</small></div>) : <p className="muted">Дневная статистика появится после первой синхронизации worker.</p>}</div>
    </article>
