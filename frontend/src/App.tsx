...
function AnalyticsPage({ analytics, subgram }: { analytics: Analytics | null; subgram: SubgramStatistics | null }) {
  ...
  const heat = (value: number | null) => value === null ? "transparent" : `rgba(110,242,189,${Math.min(0.14 + (value / 100) * 0.72, 0.86)})`;
  ...
  let cumulative = 0;
  const growthSeries = analytics.days.map((day) => { cumulative += day.bot_starts; return cumulative; });
  const growthMax = Math.max(...growthSeries, 1);
  const growthPoints = growthSeries.map((value, index) => `${(index / Math.max(growthSeries.length - 1, 1)) * 600},${150 - (value / growthMax) * 130}`).join(" ");
  const donutTotal = analytics.donation_stars_total + analytics.donation_ton_total;
  const starsShare = donutTotal ? analytics.donation_stars_total / donutTotal : 0;
  ...
  return <section className="page-stack"><SubgramAnalyticsPanel value={subgram}/><div className="page-intro"><div><p className="eyebrow">PRODUCT ANALYTICS</p><h2>Бот и подключение</h2><p>События без хранения ссылок подписки: старты бота, переходы на сайт, импорт в HAPP и выдачи VPN.</p></div><span className="status-dot">14 ДНЕЙ</span></div>
    ...
    <article className="panel"><div className="panel-head"><div><p className="eyebrow">GROWTH</p><h3>Накопительный рост новых пользователей</h3></div><span className="muted">за {analytics.days.length} дней: +{growthSeries.at(-1) || 0}</span></div><div className="area-chart"><svg viewBox="0 0 600 160" preserveAspectRatio="none"><defs><linearGradient id="growthFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#6ef2bd" stopOpacity=".35"/><stop offset="100%" stopColor="#6ef2bd" stopOpacity="0"/></linearGradient></defs><polygon points={`0,160 ${growthPoints} 600,160`} fill="url(#growthFill)"/><polyline points={growthPoints} fill="none" stroke="#6ef2bd" strokeWidth="2.5"/></svg></div></article>
    <article className="panel"><div className="panel-head"><div><p className="eyebrow">CONVERSION FUNNEL</p><h3>/start → VPN → страница → импорт → использование</h3></div><span className="muted">последовательные уникальные пользователи</span></div>
      <div className="funnel-bars">{funnel.map(([label, value], index) => {
        const previous = index === 0 ? null : funnel[index - 1][1];
        const conversion = index === 0 ? 100 : (previous ? Math.round(value * 1000 / previous) / 10 : 0);
        const width = funnel[0][1] ? Math.max((value / funnel[0][1]) * 100, 4) : 4;
        return <div className="funnel-row" key={label}><small>{label}</small><div className="funnel-track"><div className="funnel-fill" style={{ width: `${width}%` }}/><strong>{value}</strong>{index > 0 && <em>{conversion}% от прошлого шага</em>}</div></div>;
      })}</div></article>
    <article className="panel"><div className="panel-head"><div><p className="eyebrow">GEOGRAPHY</p><h3>Топ-страны активного пула</h3></div><span className="muted">живые ноды прямо сейчас</span></div>
      {(analytics.top_regions?.length ?? 0) === 0 ? <Empty text="Активный пул пока пуст — страны появятся после проверки нод."/> : <div className="region-bars">{(() => { const top = Math.max(...(analytics.top_regions || []).map((item) => item.count), 1); return (analytics.top_regions || []).map((item) => <div className="region-row" key={item.region}><span className="region-flag">{item.emoji}</span><strong>{item.region}</strong><div className="region-track"><i style={{ width: `${Math.max((item.count / top) * 100, 6)}%` }}/>{item.count}</div></div>); })()}</div>}
    </article>
    <article className="panel"><div className="panel-head"><div><p className="eyebrow">VOLUNTARY SUPPORT</p><h3>Поддержка проекта</h3></div><span className="muted">Stars и подтверждённые TON-транзакции</span></div>
      <div className="donation-split">
        <div className="donut-wrap">{donutTotal ? <svg viewBox="0 0 120 120" className="donut"><circle cx="60" cy="60" r="52" fill="none" stroke="rgba(255,255,255,.06)" strokeWidth="14"/><circle cx="60" cy="60" r="52" fill="none" stroke="#feca68" strokeWidth="14" strokeDasharray={`${starsShare * circumference} ${circumference}`} strokeLinecap="butt" transform="rotate(-90 60 60)"/><text x="60" y="57" textAnchor="middle" className="donut-value">{Math.round(starsShare * 100)}%</text><text x="60" y="73" textAnchor="middle" className="donut-label">Stars</text></svg> : <div className="empty"><p>Пока нет поддержавших.</p></div>}</div>
        <div className="donation-grid"><div><small>Открыли раздел</small><strong>{analytics.donation_opens}</strong><span>за последние 14 дней</span></div><div><small>Поддержали</small><strong>{analytics.donation_supporters}</strong><span>{donationConversion}% от открытий</span></div><div><small>Telegram Stars</small><strong>{analytics.donation_stars_total} ⭐</strong><span>{analytics.donation_stars_count} платежей</span></div><div><small>TON</small><strong>{analytics.donation_ton_total} TON</strong><span>{analytics.donation_ton_count} транзакций</span></div></div>
      </div></article>
    <article className="panel"><div className="panel-head"><div><p className="eyebrow">DAILY COHORTS</p><h3>Удержание пользователей</h3></div><span className="muted">активность в точный день после первого /start · чем ярче, тем лучше</span></div><div className="table-wrap heat-table"><table><thead><tr><th>Когорта</th><th>Юзеры</th><th>D0</th><th>D1</th><th>D3</th><th>D7</th></tr></thead><tbody>{analytics.cohorts.map((cohort) => <tr key={cohort.date}><td><strong>{new Intl.DateTimeFormat("ru-RU", { day: "numeric", month: "short" }).format(new Date(`${cohort.date}T00:00:00Z`))}</strong></td><td>{cohort.users}</td>{([cohort.d0, cohort.d1, cohort.d3, cohort.d7] as Array<number | null>).map((value, index) => <td key={index}><span className="heat-cell" style={{ background: heat(value), color: value !== null && value >= 45 ? "#08231a" : undefined }}>{retention(value)}</span></td>)}</tr>)}</tbody></table></div></article>
