...
function NodeTable({ nodes }: { nodes: Node[] }) { return <div className="table-wrap"><table><thead><tr><th>Регион и сеть</th><th>Протокол</th><th>Ping / скорость</th><th>Проверка Xray</th><th>Рейтинг</th><th>Статус</th></tr></thead><tbody>{nodes.map((node) => <tr key={node.id}><td><strong>{node.region_emoji} {node.region}</strong><small>{node.network_emoji} {node.network_label} · {node.host}:{node.port}</small></td><td><span className="protocol">{node.protocol}</span></td><td>{node.avg_latency_ms ? `${Math.round(node.avg_latency_ms)} мс` : "—"}<small>{node.probe_throughput_kbps ? `${Math.round(node.probe_throughput_kbps)} Кбит/с` : "скорость —"}</small></td><td><strong>{node.probe_stage ?? "ожидает"}</strong><small title={node.probe_error ?? ""}>{node.probe_error ? node.probe_error.slice(0, 48) : `${node.success_checks} успешно / ${node.failed_checks} ошибок`}</small></td><td><div className="score"><span style={{ width: `${Math.min(node.score, 100)}%` }}/><b>{node.score}</b></div></td><td><span className={`state ${node.state}`}>{node.state}</span></td></tr>)}</tbody></table>{nodes.length === 0 && <Empty text="Нод ещё нет: добавьте источник и запустите обработку."/>}</div>; }
